...
fun NavGraph(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { if (currentRoute == Screen.SchoolDetailsScreen.route){ Image(painter = painterResource(id = R.drawable.unilogo), contentDescription = "Unistore Logo", modifier = Modifier.size(160.dp).padding(start = 5.dp, end = 8.dp)) } },
                navigationIcon = {
                    // AJ
                    ...
                    } else if (currentRoute != Screen.SchoolDetailsScreen.route) { IconButton(onClick = { navController.popBackStack() }) { Icon(Icons.Default.ArrowBack, contentDescription = "Back") } }
                },
                actions = {
                    // Show the person icon only on the OrderDetailsScreen
                    if (currentRoute == Screen.OrderDetailsScreen.route) { IconButton(onClick = { navController.navigate(Screen.StudentDetailsScreen.route) }) { Icon(Icons.Default.Person, contentDescription = "Student Details") } }
                    // Cart icon with badge
                    IconButton(onClick = { navController.navigate(Screen.CartScreen.route) }) {
                        BadgedBox(badge = { Badge { Text(mainViewModel.cartViewModel.cartItems.collectAsState().value.size.toString()) } }) { Icon(Icons.Default.ShoppingCart, contentDescription = "Cart") }
                    }
                }
            )
        }
